...
The API gateway uses Redis for session-scoped analysis history. Redis is not exposed to the frontend.
...
History is limited to 50 records per browser session and expires after seven days. If Redis is unavailable, security analysis continues normally, but history is unavailable.
